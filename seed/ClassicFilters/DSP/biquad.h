...
public:
    ...
    Biquad(const string f):form(f)
    {
        aCoeffVector = { 0,0,0 };
        bCoeffVector = { 0,0,0 };
    }
    Biquad(vector<float> aCoeff, vector<float> bCoeff, int numCoeff, string f = "direct") :
    ...
    virtual void resetCoeff()
    {
        ...
        for (auto i = 0; i < aCoeffVector.size(); ++i)
        {
            aCoeffVector[i] = 0.0f;
            bCoeffVector[i] = 0.0f;
        }
